"""
dataset.py — N-MNIST loader using tonic for cortical_evolve experiments.

Uses tonic.datasets.NMNIST with first_saccade_only=True (default) for event
loading and tonic.transforms.ToFrame for temporal binning. Spatial pooling and
binarization are applied as a numpy step inside the tonic Compose pipeline.

Expected dataset layout (tonic manages download/caching):
    <data_root>/NMNIST/{Train,Test}/<class_id>/*.bin
