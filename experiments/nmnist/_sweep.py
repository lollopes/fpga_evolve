"""
experiments/nmnist/_sweep.py
Shared wandb sweep logic for all N-MNIST experiments.
"""
...
def _apply_overrides(base: dict, overrides: dict) -> dict:
    """
    Apply dot-notation overrides onto a nested config dict.
    e.g. {"neat.lut_bit_rate": 0.05} sets config["neat"]["lut_bit_rate"] = 0.05
    """
...
def run_sweep(base_config: dict, sweep_json: dict, exp_dir: Path,
    ...
    project    = sweep_json.get("project", f"eio-neat-nmnist-{base_config['task']}")
    ...
    def train():
        with wandb.init() as run:
            ...
            wandb.summary["best_test_acc"]  = result.get("test_acc", 0.0)
